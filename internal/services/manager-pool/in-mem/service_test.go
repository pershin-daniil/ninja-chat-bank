...
func (s *ServiceSuite) TestMaxManagersExceeded() {
	managersMax := 1000

	for i := 0; i < managersMax; i++ {
		err := s.pool.Put(s.Ctx, types.NewUserID())
		s.Require().NoError(err)
	}

	err := s.pool.Put(s.Ctx, types.NewUserID())
	s.Require().ErrorIs(err, inmemmanagerpool.ErrManagerCapacityExceeded)
}
