...
import (
	...
	storeproblem "github.com/pershin-daniil/ninja-chat-bank/internal/store/problem"
...
func (s *ProblemsRepoSuite) Test_GetManagerOpenProblemsCount() {
	...
	s.Run("manager has open problems", func() {
		...
		for i := 0; i < problemsCount; i++ {
			chatID, pID := s.createChatWithProblemAssignedTo(managerID)
			s.Require().False(chatID.IsZero())
